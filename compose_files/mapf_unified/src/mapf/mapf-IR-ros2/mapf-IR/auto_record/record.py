...
if __name__ == "__main__":
    ...
    cp = subprocess.run(
        "git --no-pager log -n 1 --no-decorate",
        shell=True,
        encoding="utf-8",
    ...
    suboptimal_res = get_run_result(
        suboptimal_ins, suboptimal_solvers, suboptimal_solvers_option
    )
    optimal_res = get_run_result(optimal_ins, optimal_solvers)
    anytime_res = get_run_result(
        anytime_ins, anytime_solvers, [anytime_solvers_option] * len(anytime_solvers)
    )
